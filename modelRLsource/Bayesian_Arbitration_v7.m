function [myArbitrator myState1 myState2]=Bayesian_Arbitration_v7(myArbitrator, myState1, myState2, myMap, mode)
...
% myArbitrator.var_cogload = 1 or 2
...
% (cognitive load factor) myArbitrator.C_12, myArbitrator.C_21
% (cognitive load factor interaction mode) myArbitrator.opt_cogload - 0: interaction ignored, 1: interacting with A, 2: interaction with B
...
% %     myArbitrator.m2_inv_Fano=(1.0)./myArbitrator.m2_var;
    ...
    if mode.ori == 2
        myArbitrator.m2_absPEestimate(myState2.goal_color_ind)=myArbitrator.m2_absPEestimate(myState2.goal_color_ind)+myArbitrator.m2_absPEestimate_lr*(abs(myState2.RPE_history(myState2.index,myState2.goal_color_ind))-myArbitrator.m2_absPEestimate(myState2.goal_color_ind));
        myArbitrator.m2_inv_Fano=[0; (10-myArbitrator.m2_absPEestimate(myState2.goal_color_ind))/10; 0]; % [0,1]. the smaller the error estimate, the more reliable
        myArbitrator.m2_mean=myArbitrator.m2_inv_Fano; myArbitrator.m2_var=[0.1; (myArbitrator.m2_absPEestimate(myState2.goal_color_ind)*0.01); 0.1];% some arbitrary settting to fill up the variables
        myArbitrator.m2_inv_Fano_color(myState2.goal_color_ind) = myArbitrator.m2_inv_Fano(2);
    else
        ...
        myArbitrator.m2_inv_Fano=[0; (10-myArbitrator.m2_absPEestimate)/10; 0]; % [0,1]. the smaller the error estimate, the more reliable
...
complx_interaction12=(1+myArbitrator.C_12*(myArbitrator.var_cogload-1));
complx_interaction21=(1+myArbitrator.C_21*(myArbitrator.var_cogload-1));
amplitude12=myArbitrator.A_12;  steepness12=myArbitrator.B_12;
amplitude21=myArbitrator.A_21;  steepness21=myArbitrator.B_21;
switch myArbitrator.opt_cogload
    
    case 0 % no interaction
    
    case 1 % interacting with A
        complx_interaction12=(1+myArbitrator.C_12*(myArbitrator.var_cogload/2));
        complx_interaction21=(1+myArbitrator.C_21*(myArbitrator.var_cogload/2));
        amplitude12=amplitude12*complx_interaction12;
        amplitude21=amplitude21*complx_interaction21;
    case 1.12 % interacting with A12 only but not A21
        complx_interaction12=(1+myArbitrator.C_12*(myArbitrator.var_cogload));
        amplitude12=amplitude12*complx_interaction12;
    case 1.21 % interacting with A21 only but not A12
        complx_interaction21=(1+myArbitrator.C_21*(myArbitrator.var_cogload));
        amplitude21=amplitude21*complx_interaction21;
    
    case 2 % interacting with B
        steepness12=steepness12*complx_interaction12;
        steepness21=steepness21*complx_interaction21;
    case 2.12 % interacting with B12 only but not B21
        steepness12=steepness12*complx_interaction12;
    case 2.21 % interacting with B21 only but not B12
        steepness21=steepness21*complx_interaction21;
    case 3
        complx_interaction12=(myArbitrator.var_cogload/2);
        complx_interaction21=(myArbitrator.var_cogload/2);
        amplitude12=amplitude12*complx_interaction12;
        amplitude21=amplitude21*complx_interaction21;
...
input0=myArbitrator.m1_inv_Fano;    input1=myArbitrator.m1_wgt'*myArbitrator.m1_inv_Fano;    % added [SW MAR 8]
myArbitrator.transition_rate12=amplitude12/(1+exp(steepness12*input1/sum(input0)));
...
myArbitrator.transition_rate21=amplitude21/(1+exp(steepness21*input2/sum(input0)));
...
%     if(sum(isnan(myArbitrator.Q(state_current,:)))>0)
%     save zzz myArbitrator
%     disp('saved')
%     end    
    q_in=(myMap.action_valid(myArbitrator.index,:)).*myArbitrator.Q(state_current,:);
    var_exp=exp(myArbitrator.tau_softmax*q_in); % (N_actionx1)
    ...
    myArbitrator.action_history(myArbitrator.index)=find(bincounts(1:end-1)>0);
end
